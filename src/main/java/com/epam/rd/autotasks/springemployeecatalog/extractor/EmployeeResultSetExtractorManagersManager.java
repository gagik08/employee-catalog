...
public class EmployeeResultSetExtractorManagersManager implements ResultSetExtractor<List<Employee>> {
    ...
    @Autowired
    public EmployeeResultSetExtractorManagersManager(MapperComponent mapperComponent) {
    ...
    public List<Employee> extractData(ResultSet resultSet) throws SQLException, DataAccessException {
        ...
        while (resultSet.next()) {
            employees.add(mapperComponent.getEmployee(resultSet, mapperComponent.getManagersManager(resultSet)));
        }
        return employees;
    }
}